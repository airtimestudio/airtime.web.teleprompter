...
(function () {
  ...
  function init() {
    ...
    document
      ...
      .setAttribute("onchange", "setStyleEvent(value);");
    ...
    slider[0].on("change", function (input) {
      document.getElementById("speedValue").textContent = parseFloat(
        Math.round(input.newValue * 10) / 10
      ).toFixed(1);
    });
    slider[1].on("change", function (input) {
      document.getElementById("accelerationValue").textContent = parseFloat(
        Math.round(input.newValue * 100) / 100
      ).toFixed(2);
    });
    ...
    if (inElectron()) {
        ...
        function (item) {
          if (item == null || compare(currentVersion, item) == 1) {
            ...
            if (!isADevVersion(item) && isADevVersion(currentVersion)) {
              ...
              document.getElementById("cancelWarningButton").onclick =
                closeWindow;
      ...
      ipcRenderer.on("asynchronous-reply", function (event, arg) {
        ...
        else if (arg.option === "qr") addQRConnection(arg.data);
        // Restore instances
        else if (arg.option === "restoreEditor") restoreEditor();
        // Forward remote control commands.
        else if (arg.option === "command") document.onkeydown(arg.data);
        //
        // Get the "exteral" classes and update each link to load on an actual browser.
          ...
          for (var i = 0; i < classTags.length; i++)
            if (classTags[i].href != " ") {
              classTags[i].setAttribute(
                "onclick",
                "shell.openExternal('" + classTags[i].href + "'); return false;"
              );
              classTags[i].href = "#";
    ...
    switch (version) {
      ...
      case "2.2.0":
        dataManager.getItem(
          ...
          function (dataToMigrate) {
            if (dataToMigrate) {
              // Convert Data
              dataToMigrate = JSON.parse(dataToMigrate);
              if (dataToMigrate.length > 0) {
                // Fix to not do more dirty work
                dataToMigrate[0]["id"] = sidebar.createIDTag(
                  dataToMigrate[0].name,
                  true
                );
                sidebar
                  .getSaveMode()
                  .setItem(sidebar.getDataKey(), JSON.stringify(dataToMigrate));
              }
              // Continue with rest of the data
              for (var i = 1; i < dataToMigrate.length; i++)
                if (dataToMigrate[i].hasOwnProperty("name")) {
                  dataToMigrate[i]["id"] = sidebar.createIDTag(
                    dataToMigrate[i].name
                  );
                  sidebar
                    .getSaveMode()
                    .setItem(
                      sidebar.getDataKey(),
                      JSON.stringify(dataToMigrate)
                    );
                }
            }
          },
  ...
  if (typeof tinymce !== "undefined") {
    tinymce.init({
      ...
      plugins:
        "advlist anchor save charmap code colorpicker contextmenu directionality emoticons fullscreen hr image media lists nonbreaking paste print searchreplace spellchecker table textcolor wordcount imagetools insertdatetime",
      toolbar: [
        "anchor | save | undo redo | styleselect | bold italic underline strikethrough | superscript subscript | forecolor backcolor | bullist numlist | alignleft aligncenter alignright | charmap image | searchreplace fullscreen",
      ],
      ...
      menu: {
        ...
        format: {
          title: "Format",
          items:
            "bold italic underline strikethrough | superscript subscript | formats | removeformat | ltr rtl",
        },
        table: {
          title: "Table",
  ...
  function launchIntoFullscreen(element) {
    ...
    if (requestFullscreen !== undefined) requestFullscreen.call(element);
  }
  ...
  function exitFullscreen() {
    ...
    if (exitFullscreen !== undefined) exitFullscreen.call(document);
  }
  ...
  function togglePrompter() {
    if (promptIt.onclick === submitTeleprompter) submitTeleprompter();
    else restoreEditor();
  }
  ...
  function internalCredits() {
    ...
    // Set data to send.
    var settings =
        '{ "data": {"secondary":0,"primary":1,"prompterStyle":2,"focusMode":3,"background":"#3CC","color":"#333","overlayBg":"#333","speed":"13","acceleration":"1.2","fontSize":"100","promptWidth":"84","timer":"false","voice":"false"}}',
    ...
    if (override !== undefined && override.timer !== undefined)
      ...
      if (
        document
          ...
          .children[0].classList.contains("btn-primary")
      )
        timer = true;
      else timer = false;
    }
  ...
  function submitTeleprompter(event) {
    ...
    if (inElectron()) {
      ...
      for (var i = 0; i < displays.length; i++) {
        if (
          !(
            displays[i].bounds.x === primaryDisplay.bounds.x &&
            displays[i].bounds.y === primaryDisplay.bounds.y
          )
        ) {
      ...
      if (
        cursorLocation.x < primaryDisplay.bounds.x ||
        cursorLocation.x > primaryDisplay.bounds.width ||
        cursorLocation.y < primaryDisplay.bounds.y ||
        cursorLocation.y > primaryDisplay.bounds.height
      )
        currentDisplay = 1;
          ...
          if (currentDisplay === 0) {
            ...
            // Otherwise run perfect sync painter.
              ...
              prompterWindow = window.open(
                "teleprompter.html" + (debug ? "?debug=1" : ""),
                "TelePrompter Output",
                "height=" +
                  (secondaryDisplay.workArea.height - 50) +
                  ",width=" +
                  (secondaryDisplay.workArea.width - 50) +
                  ",top=" +
                  (secondaryDisplay.workArea.y + 50) +
                  ",left=" +
                  (secondaryDisplay.workArea.x + 50) +
                  ",fullscreen=1,status=0,location=0,menubar=0,toolbar=0"
              );
            ...
            // Otherwise run perfect sync painter.
              ...
              prompterWindow = window.open(
                ...
                "TelePrompter Output",
                "height=" +
                  (primaryDisplay.workArea.height - 50) +
                  ",width=" +
                  (primaryDisplay.workArea.width - 50) +
                  ",top=" +
  ...
  function toggleDebug() {
    if (inElectron()) remote.getCurrentWindow().toggleDevTools();
    else toggleDebugMode();
  }
  ...
  function clearAllRequest() {
    if (
      confirm(
        "You've pressed F6. Do you wish to perform a factory reset of Teleprompter? You will loose all saved scripts and custom styles."
      )
    ) {
  ...
  function listener(event) {
    ...
    if (!event.domain || event.domain === getDomain()) {
      ...
      if (message.request === command.restoreEditor) restoreEditor();
      else {
        if (
          syncMethod === syncMethods.canvas &&
          instance[0] &&
          instance[1] &&
          inElectron()
        ) {
          ...
          if (
            !(
              message.request === command.iSync ||
              message.request === command.sync
            )
          ) {
            // Tic toc mechanism symmetricaly distributes message request lag.
            if (tic) {
              // Redirect message to each prompter instance.
              if (instance[1]) prompterWindow.postMessage(message, getDomain());
              if (instance[0])
  ...
  document.onkeydown = function (event) {
    ...
    if (event.key === undefined) event.key = event.keyCode;
    if (!editorFocused) {
      ...
      switch (event.key) {
        ...
        case "F5":
          if (debug) refresh();
            ...
            console.log(
              "Debug mode must be active to use 'F5' refresh in Electron. 'F10' enters and leaves debug mode."
            );
          break;
        case 117:
        case "F6":
        ...
        default:
          ...
          if (!is_int(key)) key = key.toLowerCase();
  ...
  function closeModal() {
    if (window.location.hash.slice(1) === "openCustomStyles")
      ...
      var version = function (thisVersion) {
        ...
        if (thisVersion === currentVersion) window.location = "#close";
        else window.close();
      };
  ...
  function updateFont(value) {
    ...
    document.getElementById("prompt").style.fontSize =
      "calc(5vw * " + value / 100 + ")";
  }
  ...
  function loadLastUseSettings() {
    ...
    var settings = function (lastSettings) {
      if (lastSettings !== undefined && lastSettings !== null) {
        ...
        else lastSettings.data.speed = slider[0].getValue();
        if (!isNaN(lastSettings.data.acceleration))
          slider[1].setValue(lastSettings.data.acceleration);
        else lastSettings.data.acceleration = slider[1].getValue();
        if (!isNaN(lastSettings.data.fontSize))
          slider[2].setValue(lastSettings.data.fontSize);
        else lastSettings.data.fontSize = slider[2].getValue();
        if (!isNaN(lastSettings.data.promptWidth))
          slider[3].setValue(lastSettings.data.promptWidth);
        else lastSettings.data.promptWidth = slider[3].getValue();
        document.getElementById("speedValue").textContent = parseFloat(
          Math.round(lastSettings.data.speed * 10) / 10
        ).toFixed(1);
        document.getElementById("accelerationValue").textContent = parseFloat(
          Math.round(lastSettings.data.acceleration * 100) / 100
        ).toFixed(2);
        document.getElementById("fontSizeValue").textContent =
          lastSettings.data.fontSize;
        document.getElementById("promptWidthValue").textContent =
          lastSettings.data.promptWidth;
  ...
  function initScripts() {
    ...
    function save() {
      if (sid.currentElement != 0) {
        ...
        scriptsData[sid.currentElement]["data"] =
          document.getElementById("prompt").innerHTML;
        sid
          .getSaveMode()
          .setItem(sid.getDataKey(), JSON.stringify(scriptsData));
      }
    }
    ...
    CKEDITOR.on("instanceReady", function (event) {
      ...
      editor.on("key", function (event) {
        if (event.key === undefined) event.key = event.data.keyCode;
        if (debug) console.log(event.key);
        if (
          sid.instructionsAreLoaded() &&
          -1 ===
            [
              1114129, 1114177, 1114179, 1114121, 5570578, 1114337, 4456466,
              2228240, 91, 225, 27, 112, 113, 114, 115, 116, 117, 118, 119, 120,
              121, 122, 123, 45, 20, 33, 34, 35, 36, 37, 38, 39, 40,
            ].indexOf(event.key)
        ) {
