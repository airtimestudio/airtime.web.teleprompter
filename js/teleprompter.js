...
(function () {
  ...
  var unit,
    ...
    focusHeight,
    promptHeight,
    previousPromptHeight,
    screenHeight,
    previousScreenHeight,
    screenWidth,
    previousVerticalDisplacementCorrector,
    domain,
    debug,
    closing,
    cap,
  ...
  function init() {
    ...
    if (window.opener) editor = window.opener;
    else if (window.top) editor = window.top;
    ...
    customStyle = {
      ...
      overlayBg: settings.data.overlayBg,
    };
    // Get flip settings
    if (inIframe()) flip = settings.data.primary;
    else flip = settings.data.secondary;
    ...
    switch (focus) {
      case 1:
        ...
        else document.querySelector("#overlayTop").classList.add("disable");
        break;
      case 2:
        if (flipV)
          document.querySelector("#overlayTop").classList.add("disable");
        else document.querySelector("#overlayBottom").classList.add("disable");
        break;
      case 4:
    ...
    window.setTimeout(function () {
      ...
      window.setTimeout(function () {
        ...
        for (var i = 0; i < 2; i++) increaseVelocity();
        instaSync();
      }, transitionDelays * 4.2);

      //Init Remote Controllers
      if (isMobileApp) remoteControls();
    }, 750);
  }
  ...
  function updateContents() {
    ...
    if (oldFontSize !== fontSize) updateFont();
    if (oldPromptWidth !== promptWidth) updateWidth();
    ...
    if (settings.data.timer === true) {
      if (timer.data().timer.currentVal === 0) timer.startTimer();
  ...
  function pointerInactive() {
    if (pointer.active) {
      if (!pointer.moved) toggleTouchControls();
  ...
  function syncPrompters() {
    editor.postMessage(
      { request: command.sync, data: getProgress() },
      getDomain()
    );
  }
  ...
  function instaSync() {
    if (steps > syncDelay)
      editor.postMessage(
        { request: command.iSync, data: getProgress() },
        getDomain()
      );
  }
  ...
  function updateVelocity() {
    // (|x|^sensitivity) * (+|-)
    velocity =
      speedMultip * Math.pow(Math.abs(x), sensitivity) * (x >= 0 ? 1 : -1);
    if (debug)
      setTimeout(function () {
  ...
  function atEnd() {
    var flag;
    if (flipV) flag = getCurrPos() >= 0;
    else flag = getCurrPos() <= -(promptHeight - screenHeight);
  ...
  function atStart() {
    var flag;
    if (flipV) flag = getCurrPos() <= -(promptHeight - screenHeight);
    else flag = getCurrPos() >= 0;
...
*/
  ...
  function getRemainingTime(destination, currPos) {
    return velocity
      ? Math.abs((1000 * (destination - currPos)) / (velocity * unit))
      : 0;
  }
  ...
  function setFocusHeight() {
    if (overlayFocus !== undefined) focusHeight = overlayFocus.clientHeight;
    else focusHeight = 0;
  }
  ...
  function getCurrPos(obj) {
    ...
    else return prompt.getBoundingClientRect().top;
  }

  function setCurrPosStill(theCurrPos) {
    if (theCurrPos === undefined) theCurrPos = getCurrPos();
  ...
  function getDestination(currPos) {
    ...
    if (velocity > 0) {
      if (flipV) whereTo = 0;
      else whereTo = -(promptHeight - screenHeight);
    } else if (velocity < 0) {
      if (flipV) whereTo = -(promptHeight - screenHeight);
      else whereTo = 0;
    }
    // Destination equals current position in animation.
    else whereTo = currPos;
    return whereTo;
  }
  ...
  function moveToCSSAnchor(theAnchor) {
    ...
    if (flipV)
      jump =
        -promptHeight +
        document.getElementById(theAnchor).offsetTop +
        screenHeight -
        focusVerticalDisplacementCorrector();
    else
      jump =
  ...
  function internalMoveToAnchor(theAnchor) {
    ...
    if (document.getElementById(theAnchor)) moveToCSSAnchor(theAnchor);
    else if (debug) console.log("Invalid Anchor") && false;
  }

  function moveToAnchor(theAnchor) {
    editor.postMessage(
      { request: command.anchor, data: theAnchor },
      getDomain()
    );
  }
  ...
  function internalMoveToNextAnchor(next) {
    ...
    if (
      (!flipV && jump < 0) ||
      (flipV && jump > -promptHeight + screenHeight) ||
      (!flipV && !next) ||
      (flipV && !next)
    ) {
      animate(0, jump);
  ...
  function internalFastForward() {
    ...
    } else if (currPos > -promptHeight + screenHeight * 1.5) {
      // < 0 || flipV && jump > -promptHeight + screenHeight || !flipV && !next || flipV && !next ) {
      const nextPos = currPos - screenHeight / 2;
      animate(0, nextPos);
      resumeAnimation();
      timeout(0.5, syncPrompters);
  ...
  function internalRewind() {
    const currPos = getCurrPos();
    if (flipV) {
      if (currPos > -promptHeight + screenHeight * 1.5) {
        // < 0 || flipV && jump > -promptHeight + screenHeight || !flipV && !next || flipV && !next ) {
        const nextPos = currPos - screenHeight / 2;
        animate(0, nextPos);
        resumeAnimation();
    ...
    } else if (currPos < 0 - screenHeight * 0.5) {
      // < 0 || flipV && jump > -promptHeight + screenHeight || !flipV && !next || flipV && !next ) {
      const nextPos = currPos + screenHeight / 2;
      animate(0, nextPos);
      resumeAnimation();
  ...
  function updateUnit() {
    ...
    if (debug)
      setTimeout(function () {
        console.log("Unit updated: " + unit) && false;
      });
    resumeAnimation();
  }
  ...
  function timeout(time, func) {
    ...
    if (timeoutStatus) window.clearTimeout(timeoutStatus);
  ...
  function getProgress() {
    ...
    if (flipV)
      progress =
        (-(previousPromptHeight - previousScreenHeight + currPos) -
          previousVerticalDisplacementCorrector +
          previousScreenHeight) /
        -(previousPromptHeight - previousScreenHeight * 2);
    else
      progress =
        (currPos -
          previousVerticalDisplacementCorrector +
  ...
  function correctVerticalDisplacement(percentage, transitionDelay) {
    ...
    animationTimeout(delay, function () {
      ...
      if (percentage === undefined) percentage = getProgress();
      // Solve
      if (flipV)
        updatedPos =
          -(
            -percentage * (promptHeight - screenHeight * 2) +
            valToCenterAtFocusArea -
            screenHeight
          ) -
          promptHeight +
          screenHeight;
  ...
  window.addEventListener(
    ...
    function (event) {
      ...
      if (invertedWheel) {
        if (event.deltaY > 0) increaseVelocity();
        else decreaseVelocity();
      } else {
        if (event.deltaY > 0) decreaseVelocity();
        else increaseVelocity();
      }
  ...
  function listener(event) {
    ...
    if ((!cap && !event.domain) || event.domain === getDomain()) {
      ...
      if (
        message.request == command.increaseVelocity ||
        message.request == command.decreaseVelocity
      )
        setCap();
      switch (message.request) {
        ...
        default:
          ...
          if (debug)
            console.log("Unknown post message received: " + message.request) &&
              false;
      }
    }
  }
